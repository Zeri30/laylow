...
void main() {
  group('deezerSearchTermsFor', () {
    ...
    test('clamps out-of-range intensity instead of throwing', () {
      ...
      expect(
        deezerSearchTermsFor('calm', 0),
        equals(deezerSearchTermsFor('calm', 1)),
      );
      expect(
        deezerSearchTermsFor('calm', 99),
        equals(deezerSearchTermsFor('calm', 5)),
      );
    });
    ...
    test(
      'always returns 4 terms with no duplicates, sliding across intensity',
      () {
        for (final mood in moodOptions) {
